...
const queueNudgeDuelSignalConsensusProtocolFlowLabel = "Duel Signal Flow";
...
<html lang="en">
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      .hero-story .duel-signal-consensus-protocol-flow-inline {
        margin-top: 6px;
        display: inline-flex;
        align-items: center;
        gap: 6px;
      ...
      .hero-story .duel-signal-consensus-protocol-flow-inline.call-lead {
        border-color: #86efac;
        background: #ecfdf5;
        color: #166534;
      }
      .hero-story .duel-signal-consensus-protocol-flow-inline.call-rival {
        border-color: #bfdbfe;
        background: #eff6ff;
        color: #1d4ed8;
      }
      .hero-story .duel-signal-consensus-protocol-flow-inline.call-hold {
        border-color: #fecaca;
        background: #fff1f2;
        color: #9f1239;
      }
      ...
      .aha-nudge .nudge-duel-signal-consensus-protocol-flow {
        display: inline-flex;
        align-items: center;
        gap: 6px;
        border-radius: 999px;
      ...
      .aha-nudge .nudge-duel-signal-consensus-protocol-flow.call-lead {
        border-color: #86efac;
        background: #ecfdf5;
        color: #166534;
      }
      .aha-nudge .nudge-duel-signal-consensus-protocol-flow.call-rival {
        border-color: #bfdbfe;
        background: #eff6ff;
        color: #1d4ed8;
      }
      .aha-nudge .nudge-duel-signal-consensus-protocol-flow.call-hold {
        border-color: #fecaca;
        background: #fff1f2;
        color: #9f1239;
      }
  ...
  <body>
    ...
    <script>
      ...
      const QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL = ${JSON.stringify(queueNudgeDuelSignalConsensusProtocolFlowLabel)};
      ...
      const QUEUE_NUDGE_COPY_SIGNAL_FLOW_LABEL = ${JSON.stringify(queueNudgeCopySignalFlowLabel)};
      ...
      const QUEUE_NUDGE_DOWNLOAD_SIGNAL_FLOW_LABEL = ${JSON.stringify(queueNudgeDownloadSignalFlowLabel)};
      ...
      function ahaDuelSignalConsensusProtocolFlowMeta(
        poolItems = null,
        trigger = null,
        protocol = null,
        command = null,
        ...
        const resolvedTrigger = trigger || ahaDuelSignalConsensusTriggerMeta(ranked);
        const resolvedProtocol = protocol || ahaDuelSignalConsensusProtocolMeta(ranked);
        const resolvedCommand = command || ahaDuelSignalConsensusProtocolCommandMeta(ranked, resolvedTrigger, resolvedProtocol);
        const resolvedScript = script || ahaDuelSignalConsensusProtocolScriptMeta(ranked, resolvedCommand);
        const resolvedChecklist =
          checklist ||
          ahaDuelSignalConsensusProtocolScriptChecklistMeta(
            ranked,
            resolvedScript,
            null,
            resolvedCommand,
          );
        if (!resolvedTrigger || !resolvedProtocol || !resolvedCommand || !resolvedScript || !resolvedChecklist) return null;
        const triggerLabel = String(resolvedTrigger.label || "").toLowerCase();
        const commandLabel = String(resolvedCommand.label || "").toLowerCase();
        const checklistLabel = String(resolvedChecklist.label || "").toLowerCase();
        if (triggerLabel.includes("no trigger") || checklistLabel.includes("hold")) {
          return { label: "Flow standby", tone: "call-hold", hint: resolvedTrigger.label + " · " + resolvedChecklist.label };
        }
        if (commandLabel.includes("split") || checklistLabel.includes("replay")) {
          return { label: "Flow replay", tone: "call-rival", hint: resolvedCommand.label + " · " + resolvedChecklist.label };
        }
        if (commandLabel.includes("launch") || checklistLabel.includes("launch")) {
          return { label: "Flow execute", tone: "call-lead", hint: resolvedCommand.label + " · " + resolvedScript.label };
        }
        if (commandLabel.includes("stage") || checklistLabel.includes("checkpoint")) {
          return { label: "Flow stage", tone: "call-lead", hint: resolvedCommand.label + " · " + resolvedChecklist.label };
        }
        if (commandLabel.includes("probe") || checklistLabel.includes("probe")) {
          return { label: "Flow probe", tone: "call-rival", hint: resolvedCommand.label + " · " + resolvedScript.label };
        }
        if (commandLabel.includes("watch") || checklistLabel.includes("watch")) {
          return { label: "Flow watch", tone: "call-rival", hint: resolvedCommand.label + " · " + resolvedChecklist.label };
        }
        return { label: "Flow active", tone: "call-rival", hint: resolvedProtocol.label + " · " + resolvedCommand.label };
      }
      ...
      function ahaDecisionBriefText(poolItems) {
        ...
        const duelSignalConsensusProtocolFlow = ahaDuelSignalConsensusProtocolFlowMeta();
        ...
        if (duelSignalConsensusProtocolFlow) {
          lines.push(
            QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL +
              ": " +
              duelSignalConsensusProtocolFlow.label +
              " · " +
              duelSignalConsensusProtocolFlow.hint,
          );
        }
        if (duelTrend) {
      ...
      function ahaDuelActionPlanText(poolItems) {
        ...
        const signalConsensusProtocolFlow = ahaDuelSignalConsensusProtocolFlowMeta(
          ranked,
        ...
        return (
          ...
          (signalConsensusProtocolScriptChecklist
            ? QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_SCRIPT_CHECKLIST_LABEL +
                ...
                "\\n"
            : "") +
          (signalConsensusProtocolFlow
            ? QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL +
                ": " +
                signalConsensusProtocolFlow.label +
                " · " +
                signalConsensusProtocolFlow.hint +
                "\\n"
            : "") +
      ...
      function ahaDuelCallText(poolItems) {
        ...
        return (
          ...
          + (signalConsensusProtocolScriptChecklist
            ...
            : "")
          + (signalConsensusProtocolFlow
            ? "\\n" + QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL + ": " + signalConsensusProtocolFlow.label + " · " + signalConsensusProtocolFlow.hint
            : "")
        );
      }
      ...
      function ahaDuelSignalsText(poolItems) {
        ...
        if (signalConsensusProtocolFlow) {
          lines.push(
            QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL +
              ": " +
              signalConsensusProtocolFlow.label +
      ...
      function ahaDuelSignalHandoffText(poolItems) {
        ...
        const consensusProtocolFlow = ahaDuelSignalConsensusProtocolFlowMeta(
          ranked,
          consensusTrigger,
          consensusProtocol,
          consensusProtocolCommand,
          consensusProtocolScript,
          consensusProtocolScriptChecklist,
        );
        if (!handoff) return "";
        ...
        if (consensusProtocolFlow) {
          lines.push(QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL + ": " + consensusProtocolFlow.label + " · " + consensusProtocolFlow.hint);
        }
        return lines.join("\\n");
      }
      ...
      function ahaDuelSignalConsensusProtocolText(poolItems) {
        ...
        const flow = ahaDuelSignalConsensusProtocolFlowMeta(ranked, trigger, protocol, command, script, scriptChecklist);
        ...
        if (flow) {
          lines.push(QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL + ": " + flow.label + " · " + flow.hint);
        }
        return lines.join("\\n");
      }
      ...
      function ahaDuelSignalConsensusFlowText(poolItems) {
        const ranked = sortedAhaItems(poolItems);
        if (ranked.length < 2) return "";
        const trigger = ahaDuelSignalConsensusTriggerMeta(ranked);
        const protocol = ahaDuelSignalConsensusProtocolMeta(ranked);
        ...
        const scriptLane = ahaDuelSignalConsensusProtocolScriptLaneMeta(ranked, script, command);
        const scriptChecklist = ahaDuelSignalConsensusProtocolScriptChecklistMeta(ranked, script, scriptLane, command);
        if (!trigger || !protocol || !command || !script || !scriptChecklist) return "";
        const lines = [
          "Aha Duel Signal Flow",
      ...
      function ahaDuelSnapshotText(poolItems) {
        ...
        if (consensusProtocolFlow) parts.push("consensusProtocolFlow " + consensusProtocolFlow.label);
        ...
        if (ahaPool.length > 0) {
          ...
          if (ahaPool.length > 1) {
            ...
            if (duelText) {
              ...
              const duelSignalConsensusProtocolFlow = ahaDuelSignalConsensusProtocolFlowMeta(
                ahaPool,
                duelSignalConsensusTrigger,
                duelSignalConsensusProtocol,
                duelSignalConsensusProtocolCommand,
                duelSignalConsensusProtocolScript,
                duelSignalConsensusProtocolScriptChecklist,
              );
              if (duelSignalConsensusProtocolScript) {
              ...
              if (duelSignalConsensusProtocolFlow) {
                const signalConsensusProtocolFlowEl = document.createElement("span");
                signalConsensusProtocolFlowEl.className =
                  "nudge-duel-signal-consensus-protocol-flow " + duelSignalConsensusProtocolFlow.tone;
                signalConsensusProtocolFlowEl.textContent =
                  QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL +
                  ": " +
                  duelSignalConsensusProtocolFlow.label +
                  " · " +
                  duelSignalConsensusProtocolFlow.hint;
                duelEl.appendChild(signalConsensusProtocolFlowEl);
              }
              ...
              const copySignalFlowBtn = document.createElement("button");
              copySignalFlowBtn.type = "button";
              copySignalFlowBtn.className = "secondary";
              copySignalFlowBtn.textContent = QUEUE_NUDGE_COPY_SIGNAL_FLOW_LABEL;
              copySignalFlowBtn.addEventListener("click", async () => {
                await runCopyAhaDuelSignalFlowAction(copySignalFlowBtn);
              });
              actionsEl.appendChild(copySignalFlowBtn);
              ...
              const downloadSignalFlowBtn = document.createElement("button");
              downloadSignalFlowBtn.type = "button";
              downloadSignalFlowBtn.className = "secondary";
              downloadSignalFlowBtn.textContent = QUEUE_NUDGE_DOWNLOAD_SIGNAL_FLOW_LABEL;
              downloadSignalFlowBtn.addEventListener("click", async () => {
                await runDownloadAhaDuelSignalFlowAction(downloadSignalFlowBtn);
              });
              actionsEl.appendChild(downloadSignalFlowBtn);
      ...
      function renderDetailHeroStory(item, hostEl) {
        ...
        if (duel) {
          ...
          if (duelSignalConsensusProtocolFlow) {
            const duelSignalConsensusProtocolFlowEl = document.createElement("span");
            duelSignalConsensusProtocolFlowEl.className =
              "duel-signal-consensus-protocol-flow-inline " + duelSignalConsensusProtocolFlow.tone;
            duelSignalConsensusProtocolFlowEl.textContent =
              QUEUE_NUDGE_DUEL_SIGNAL_CONSENSUS_PROTOCOL_FLOW_LABEL +
              ": " +
              duelSignalConsensusProtocolFlow.label +
              " · " +
              duelSignalConsensusProtocolFlow.hint;
            storyHost.appendChild(duelSignalConsensusProtocolFlowEl);
          }
          if (duelSnapshot) {
      ...
      async function runCopyAhaDuelSignalFlowAction(button = null) {
        await runActionWithFeedback(
          {
            id: "queue_copy_aha_duel_signal_flow",
            label: "Copy Signal Flow",
            action: async () => {
              const visibleItems = visibleQueueItems();
              const pool = visibleItems.length ? visibleItems : allItems;
              const text = ahaDuelSignalConsensusFlowText(pool);
              if (!text) {
                errorEl.textContent = "Duel signal flow is unavailable under current filters.";
                return;
              }
              const copied = await copyTextToClipboard(text, {
                success: "Copied duel signal flow.",
                failure: "Copy duel signal flow failed.",
              });
              if (!copied) {
                throw new Error("Copy duel signal flow failed.");
              }
            },
          },
          { button, localFeedbackEl: queueActionBannerEl },
      ...
      async function runDownloadAhaDuelSignalFlowAction(button = null) {
        await runActionWithFeedback(
          {
            id: "queue_download_aha_duel_signal_flow",
            label: "Download Signal Flow",
            action: async () => {
              const visibleItems = visibleQueueItems();
              const pool = visibleItems.length ? visibleItems : allItems;
              const text = ahaDuelSignalConsensusFlowText(pool);
              ...
              const fileName = "aha_signal_flow_" + new Date().toISOString().replace(/[:.]/g, "-") + ".txt";
              const blob = new Blob([text], { type: "text/plain;charset=utf-8" });
              const url = URL.createObjectURL(blob);
              const anchor = document.createElement("a");
              anchor.href = url;
              ...
              document.body.removeChild(anchor);
              URL.revokeObjectURL(url);
              errorEl.textContent = "Downloaded duel signal flow.";
            },
          },
          { button, localFeedbackEl: queueActionBannerEl },
        );
      ...
      async function runDuelSignalProtocolAction(button = null) {
        ...
        const flow = ahaDuelSignalConsensusProtocolFlowMeta(
          pool,
          trigger,
          protocol,
          protocolCommand,
          protocolScript,
          protocolScriptChecklist,
        );
        if (!trigger || !protocol) {
      ...
      async function runDuelSignalCommandAction(button = null) {
        ...
        const flow = ahaDuelSignalConsensusProtocolFlowMeta(pool, trigger, protocol, command, script, scriptChecklist);
      ...
      async function runDuelSignalFlowAction(button = null) {
        ...
        const flow = ahaDuelSignalConsensusProtocolFlowMeta(pool, trigger, protocol, command, script, scriptChecklist);
        const checklistLabel = String(scriptChecklist?.label || "").toLowerCase();
        if (!trigger || !protocol || !command || !script || !scriptChecklist || !flow) {
