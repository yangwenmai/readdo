...
* source_type 枚举：`web | youtube | newsletter | other`（大小写不敏感，服务端会归一化为小写；若显式提供则必须为非空合法枚举值；若缺省则服务端会基于 URL `hostname` 推断（hostname 会先做小写归一化并移除尾随 `.`；`youtube.com|youtu.be` 域名命中 -> youtube；`substack.com` 或 newsletter 形态子域命中 -> newsletter；其余 http(s) -> web））
...
* `q` 若提供，必须为非空字符串；按 `title/domain/intent_text/url` 模糊过滤失败候选
...
* `q` 若提供，必须为非空字符串；按 `title/domain/intent_text/url` 模糊过滤 archived 候选
* 响应中的 `scanned_total` 为匹配筛选条件的总量（未截断前），`scan_truncated=true` 表示受到 `limit` 截断
...
* `offset`：可选，默认 0；负值、空白值或非法值按 0 处理
* `cursor`：分页游标（可选；若提供必须为非空字符串）
* `limit`：默认 20，范围 `1..100`；空白值或非法值回退为默认值 20
